...
let write_event (T t) event =
  ...
  | Ok
    ...
    let how_many_call =
      List.length callstack - Stack.length thread_info.callstack.stack
    in
    let calls = List.drop callstack (Stack.length thread_info.callstack.stack) in
    let (_ : Event.Location.t list) =
      List.init how_many_call ~f:Fn.id
      |> List.fold ~init:calls ~f:(fun calls _ ->
             match calls with
             | head :: tail ->
               call t thread_info ~time ~location:head;
               tail
             | [] ->
               raise_s [%message "BUG: Unexpected behavior while parsing callstack."])
    in
    ()
  | Ok
      { Event.Ok.thread = _ (* Already used this to look up thread info. *)
