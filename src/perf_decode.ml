...
let ok_perf_sample_line_re =
  Re.Perl.re {|^ *([0-9]+)/([0-9]+) +([0-9]+).([0-9]+): +cycles:[a-zA-Z]+: *$|}
  |> Re.compile
;;
...
let ok_perf_line_re =
  Re.Perl.re
    {|^ *([0-9]+)/([0-9]+) +([0-9]+).([0-9]+): +branches:[a-zA-Z]+: +(call|return|tr strt|syscall|sysret|hw int|iret|tr end|tr strt tr end|tr end  (?:call|return|syscall|sysret|iret)|jmp|jcc) +([0-9a-f]+) (.*) => +([0-9a-f]+) (.*)$|}
  |> Re.compile
;;
...
let ok_perf_power_line_re =
  Re.Perl.re
    {|^ *([0-9]+)/([0-9]+) +([0-9]+).([0-9]+): +(cbr|psb): +([a-z]*)? +(cbr|psb offs): ([0-9]+ freq: ([0-9]+) MHz)?(.*)$|}
  |> Re.compile
;;
...
let classify line =
  ...
  then Trace_error
    ...
    let event_name = List.nth_exn split_line 1 |> String.lstrip in
    match event_name with
    | "branches" -> Ok_perf_line
    | "cycles" -> Ok_perf_sample_line
    | "cbr" | "psb" -> Ok_perf_power_line
    | _ -> raise_s [%message "Unexpected event name in perf output."])
;;
...
let trace_error_to_event line : Event.Decode_error.t =
  let matches = Re.Group.all (Re.exec trace_error_re line) in
  match matches with
...
let ok_perf_power_line_to_event line : Event.Ok.t option =
  let matches = Re.Group.all (Re.exec ok_perf_power_line_re line) in
  match matches with
  | [| _; pid; tid; time_hi; time_lo; _; _; kind; _; freq; _ |] ->
    let pid = maybe_pid_of_string pid in
    let tid = maybe_pid_of_string tid in
...
let ok_perf_sample_line_to_event ?perf_maps line lines : Event.Ok.t =
  let matches = Re.Group.all (Re.exec ok_perf_sample_line_re line) in
  match matches with
...
let ok_perf_line_to_event ?perf_maps line : Event.Ok.t =
  let matches = Re.Group.all (Re.exec ok_perf_line_re line) in
  match matches with
  | [| _
     ; pid
     ; tid
...
let to_event ?perf_maps lines : Event.t option =
  try
    ...
    | first_line :: lines ->
      ...
      | Ok_perf_line -> Some (Ok (ok_perf_line_to_event ?perf_maps first_line))
      | Ok_perf_sample_line ->
        Some (Ok (ok_perf_sample_line_to_event ?perf_maps first_line lines))
      | Ok_perf_power_line ->
        ok_perf_power_line_to_event first_line |> Option.map ~f:(fun event -> Ok event))
  ...
  (module struct
    ...
    let%expect_test "C symbol" =
      ...
      ("BUG: exception raised while parsing perf output. Please report this to https://github.com/janestreet/magic-trace/issues/"
        (exn "Unexpected event name in perf output.")
        (perf_output
          (" 25375/25375 4509191.343298468:   call                     7f6fce0b71f4 __clock_gettime+0x24 (foo.so) =>     7ffd193838e0 __vdso_clock_gettime+0x0 (foo.so)")))
      Raised at Base__Error.raise in file "src/error.ml" (inlined), line 9, characters 14-30
      Called from Base__Error.raise_s in file "src/error.ml", line 10, characters 19-40
      Called from Magic_trace_lib__Perf_decode.(fun).M.check in file "src/perf_decode.ml", line 374, characters 6-40
      Called from Magic_trace_lib__Perf_decode.(fun).M.(fun) in file "src/perf_decode.ml", line 378, characters 6-179
    ...
    let%expect_test "C symbol trace start" =
      ...
      Called from Base__Error.raise_s in file "src/error.ml", line 10, characters 19-40
      Called from Magic_trace_lib__Perf_decode.(fun).M.check in file "src/perf_decode.ml", line 374, characters 6-40
      Called from Magic_trace_lib__Perf_decode.(fun).M.(fun) in file "src/perf_decode.ml", line 395, characters 6-163
      Called from Expect_test_collector.Make.Instance_io.exec in file "collector/expect_test_collector.ml", line 262, characters 12-19 |}]
    ;;
    ...
    let%expect_test "C++ symbol" =
      ...
      Called from Base__Error.raise_s in file "src/error.ml", line 10, characters 19-40
      Called from Magic_trace_lib__Perf_decode.(fun).M.check in file "src/perf_decode.ml", line 374, characters 6-40
      Called from Magic_trace_lib__Perf_decode.(fun).M.(fun) in file "src/perf_decode.ml", line 412, characters 6-217
      Called from Expect_test_collector.Make.Instance_io.exec in file "collector/expect_test_collector.ml", line 262, characters 12-19 |}]
    ;;
    ...
    let%expect_test "OCaml symbol" =
      ...
      Called from Base__Error.raise_s in file "src/error.ml", line 10, characters 19-40
      Called from Magic_trace_lib__Perf_decode.(fun).M.check in file "src/perf_decode.ml", line 374, characters 6-40
      Called from Magic_trace_lib__Perf_decode.(fun).M.(fun) in file "src/perf_decode.ml", line 431, characters 6-178
      Called from Expect_test_collector.Make.Instance_io.exec in file "collector/expect_test_collector.ml", line 262, characters 12-19 |}]
    ;;
    ...
    let%expect_test "manufactured example 1" =
      ...
      Called from Base__Error.raise_s in file "src/error.ml", line 10, characters 19-40
      Called from Magic_trace_lib__Perf_decode.(fun).M.check in file "src/perf_decode.ml", line 374, characters 6-40
      Called from Magic_trace_lib__Perf_decode.(fun).M.(fun) in file "src/perf_decode.ml", line 460, characters 6-161
      Called from Expect_test_collector.Make.Instance_io.exec in file "collector/expect_test_collector.ml", line 262, characters 12-19 |}]
    ;;
    ...
    let%expect_test "manufactured example 2" =
      ...
      Called from Base__Error.raise_s in file "src/error.ml", line 10, characters 19-40
      Called from Magic_trace_lib__Perf_decode.(fun).M.check in file "src/perf_decode.ml", line 374, characters 6-40
      Called from Magic_trace_lib__Perf_decode.(fun).M.(fun) in file "src/perf_decode.ml", line 477, characters 6-153
      Called from Expect_test_collector.Make.Instance_io.exec in file "collector/expect_test_collector.ml", line 262, characters 12-19 |}]
    ;;
    ...
    let%expect_test "manufactured example 3" =
      ...
      Called from Base__Error.raise_s in file "src/error.ml", line 10, characters 19-40
      Called from Magic_trace_lib__Perf_decode.(fun).M.check in file "src/perf_decode.ml", line 374, characters 6-40
      Called from Magic_trace_lib__Perf_decode.(fun).M.(fun) in file "src/perf_decode.ml", line 494, characters 6-158
      Called from Expect_test_collector.Make.Instance_io.exec in file "collector/expect_test_collector.ml", line 262, characters 12-19 |}]
    ;;
    ...
    let%expect_test "unknown symbol in DSO" =
      ...
      Called from Base__Error.raise_s in file "src/error.ml", line 10, characters 19-40
      Called from Magic_trace_lib__Perf_decode.(fun).M.check in file "src/perf_decode.ml", line 374, characters 6-40
      Called from Magic_trace_lib__Perf_decode.(fun).M.(fun) in file "src/perf_decode.ml", line 511, characters 6-161
      Called from Expect_test_collector.Make.Instance_io.exec in file "collector/expect_test_collector.ml", line 262, characters 12-19 |}]
    ;;
    ...
    let%expect_test "DSO with spaces in it" =
      ...
      Called from Base__Error.raise_s in file "src/error.ml", line 10, characters 19-40
      Called from Magic_trace_lib__Perf_decode.(fun).M.check in file "src/perf_decode.ml", line 374, characters 6-40
      Called from Magic_trace_lib__Perf_decode.(fun).M.(fun) in file "src/perf_decode.ml", line 529, characters 6-178
      Called from Expect_test_collector.Make.Instance_io.exec in file "collector/expect_test_collector.ml", line 262, characters 12-19 |}]
    ;;
    ...
    let%expect_test "power event psb offs" =
      ...
      ("BUG: exception raised while parsing perf output. Please report this to https://github.com/janestreet/magic-trace/issues/"
        (exn "Unexpected event name in perf output.")
        (perf_output
          ("2937048/2937048 448556.689403475:                        psb offs: 0x4be8                                0     7f068fbfd330 mmap64+0x50 (/usr/lib64/ld-2.28.so)")))
      Raised at Base__Error.raise in file "src/error.ml" (inlined), line 9, characters 14-30
      Called from Base__Error.raise_s in file "src/error.ml", line 10, characters 19-40
      Called from Magic_trace_lib__Perf_decode.(fun).M.check in file "src/perf_decode.ml", line 374, characters 6-40
      Called from Magic_trace_lib__Perf_decode.(fun).M.(fun) in file "src/perf_decode.ml", line 606, characters 6-203
      Called from Expect_test_collector.Make.Instance_io.exec in file "collector/expect_test_collector.ml", line 262, characters 12-19 |}]
    ;;
    ...
    let%expect_test "sampled callstack" =
      ...
      Called from Base__Error.raise_s in file "src/error.ml", line 10, characters 19-40
      Called from Magic_trace_lib__Perf_decode.(fun).M.check in file "src/perf_decode.ml", line 374, characters 6-40
      Called from Magic_trace_lib__Perf_decode.(fun).M.(fun) in file "src/perf_decode.ml", line 615, characters 6-1023
      Called from Expect_test_collector.Make.Instance_io.exec in file "collector/expect_test_collector.ml", line 262, characters 12-19 |}]
    ;;
  end)
;;
