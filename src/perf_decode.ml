...
let ok_perf_header_re =
  Re.Perl.re {|^ *([0-9]+)/([0-9]+) +([0-9]+)\.([0-9]+): +([a-z]+):([a-zA-Z]+:)?(.*)$|}
  |> Re.compile
;;
...
type header =
  ...
  | Event of
      ...
      ; time : Time_ns.Span.t
      ; event : [ `Branches_event | `Cycles_event | `Cbr_event | `Psb_event ]
...
let parse_event_header line =
  ...
  then Trace_error
    ...
    match Re.Group.all (Re.exec ok_perf_header_re line) with
    | [| _str; pid; tid; time_hi; time_lo; event_name; _selector; remaining_line |] ->
      ...
      in
      Event { thread = { pid; tid }; time; event; remaining_line }
    | results ->
      raise_s
        [%message
...
let parse_perf_cbr_event thread time line : Event.t =
  match Re.Group.all (Re.exec ok_perf_power_line_re line) with
  | [| _; _; _; freq; _ |] ->
...
let to_event ?perf_maps lines : Event.t option =
  try
    ...
    | first_line :: lines ->
      ...
      | Event { thread; time; event; remaining_line } ->
        (match event with
        | `Branches_event ->
        ...
        | `Psb_event -> None
        | `Cycles_event -> Some (parse_perf_cycles_event ?perf_maps thread time lines)))
